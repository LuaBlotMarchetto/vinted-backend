...
router.post("/offer/publish",fileupload(), isAuthenticated, async(req,res)=>{
    try {
        ...
        const result = await cloudinary.uploader.upload(readablePicture, {folder: `vinted/offers/${newOffer._id}`});
...
router.put("/offers/:id",fileupload(),isAuthenticated, async(req,res)=>{
        try {
            //récuopération de l'annonce d'après son id
            ...
            const pictureToUpload = req.files.picture;
            const result = await cloudinary.uploader.upload(convertToBase64(pictureToUpload), {folder: `vinted/offers/${offer._id}`});
...
router.delete("/offers/delete/:id",async(req,res)=>{
    try {
